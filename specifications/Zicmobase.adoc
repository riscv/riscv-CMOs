...
_with respect to a set of agents._
****
...
_custom PoCs before the PoC-memory as if such operations were to the_
_PoC-memory._
****

A CMO instruction operates on the caches accessed while traversing a given 
path, and any number of caches may be accessed along a path up to the specified 
PoC.
On a path, private caches are accessed between a system agent and the first 
PoC, while shared caches are accessed between PoCs.
Memory accesses from CMO instructions access the caches on a path _directly_.

Caches introduce multiple copies of cache blocks, and a system may implement a 
_cache coherence_ mechanism in hardware to ensure that the copies in a set of 
caches remain _coherent_ with respect to each other, i.e. the copies in the set 
of caches appear to have the same data values, regardless of which cache in the 
set is accessed.
Such a set corresponds to a _coherence domain_ (or _domain_ for short), which 
may consist of any number of caches, including an individual cache, and a given 
coherence domain is defined by the same characteristics that determine the 
path, specifically the physical address and memory attributes of a memory 
access.
Additionally, depending on the cache coherence mechanism, memory accesses from 
CMO instructions _indirectly_ access the caches within a coherence domain that 
are _not_ accessed directly.
...
PoCs and coherence domains are related 



In particular, the memory attributes of a memory access define the coherence 
domain.



The PoC in a coherence domain closest to the PoC-memory is special.

A PoC is not required to correspond to a cache.
...
If two paths converge at the same PoC and the memory access



paths, convergence, domains overlay

SW view: POCs define sets of agents that communicate coherently without SW help

Coherent up to a PoC?

direct caches on path, indirect caches above PoC in coherence domain

PoC closest to memory in domain
...
Non-coherent implies that no caches are accessed indirectly.
