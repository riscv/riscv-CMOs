...
Caches may be accessed _directly_, as a result of a memory access travelling along 
a path, or _indirectly_, as a result of the cache coherence protocol



Caches are organized in a hierarchically between system agents and memory 
agents.

Physical vs. logical hierarchy

Paths converge physically at a cache, logically  at a domain.

A memory access traverses the hierarchy along a path

A memory access accesses a series of caches
...
Caches along a path are accessed _directly_. If a hardware cache coherence 
mechanism is implemented, additional caches within a coherence domain may be 
accessed _indirectly_ depending on the state of those caches.
...
model. How the operations resulting from CMO instructions fit into these rules 
is described below.
